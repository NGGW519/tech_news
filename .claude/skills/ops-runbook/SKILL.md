---
name: ops-runbook
description: "tech-news-orchestrator 가 ops-investigator 에게 시키는 하위 절차 — 사용자 요청은 오케스트레이터가 먼저 받는다. 주간 테크 뉴스 브리핑 파이프라인의 읽기 전용 운영 진단 런북(저장소 파일을 수정하지 않는다): logs/last_run.txt 필드·status 해석표, failures 단계명→모듈→흔한 원인 매핑, 증상별 진단 트리(알림 누락·미발행·0건·exit 1), 멱등성을 고려한 복구 절차, 자격 증명 수명표(전부 무기한), SPEC 11절 외부 API 사양 점검표(Notion 멘션 푸시·통합 권한·Gemini 모델 ID·네이버 API HUB·arXiv). 실행 실패·알림 누락·자격 증명·외부 사양 진단을 맡은 에이전트는 이 스킬을 읽는다. 코드 수정 규약은 pipeline-dev, SPEC 갱신은 spec-change."
---

# 운영 런북

이 파이프라인은 GitHub Actions 에서 매주 월요일 08:10 KST 에 무인 실행된다. 실패의 대부분은 코드가 아니라
바깥이 바뀐 것에서 온다 — Reddit API 정책 변경(소스 제거), 네이버 검색 API 의 API HUB 이관, 카카오 "나에게 보내기"의 푸시 미발생(v1.6에서 채널 교체)이
...
| `status` | 뜻 | 조치 |
|----------|-----|------|
| `published` | Notion 쓰기 완료 = **알림도 발송됨**. 멘션이 토글 안에 있어 둘이 분리되지 않는다 (SPEC 8절) | `failures` 확인 |
...
| (Actions 실패 + `last_run.txt` 미갱신) | `notion.py` 또는 `config.py` | Notion 토큰·루트 페이지 권한, API 버전, 환경 변수 누락(`환경 변수 누락:` SystemExit), **통합의 사용자 정보 읽기 권한 미설정 → `400 validation_error`** | Notion 은 `_guard` 밖 — 예외면 프로세스가 죽고 로그도 안 써진다 |
...
```
알림이 안 왔다
├─ **먼저 물을 것: 폰이나 PC 에서 Notion 을 열어 두고 있었는가** → 열려 있으면 푸시 대신
│     인박스 뱃지만 뜬다 (SPEC 8절 "푸시가 울리는 조건", 실측 확인). **장애가 아니라 설계된 동작이다.**
│     그 다음 폰 OS 알림 권한과 앱 내 알림 설정을 확인한다. 여기까지가 파이프라인 밖이다
├─ Notion 에 이번 주 토글 있음 + status published → **알림은 발송됐다.** 토글에 멘션 조각이
│     실제로 들어 있는지 확인한다 (`rich_text` 2조각, 1번이 라벨·2번이 mention). 없으면 코드 문제
├─ Notion 에 이번 주 토글 있음 + status already_exists → 이번 실행은 멱등성 게이트에서 끝났고
│     **Notion 쓰기도 알림도 없었다**. 토글을 만든 이전 실행의 로그를
...
├─ **Notion 에 토글이 없고 Actions 로그에 `400 validation_error` + `Could not find user with ID`**
│     → 통합의 **사용자 정보 읽기 권한 미설정** (SPEC 8절 "통합 권한"). 증상이 알림 누락이 아니라
│     **그 주 미발행**이다. 콘솔에서 「이메일 주소를 제외한 사용자 정보 읽기」로 올린 뒤 `workflow_dispatch`
...
| 통합 권한 미설정으로 `400` | 토글이 **안 만들어졌으므로** 멱등성에 안 걸린다. 콘솔에서 권한을 올린 뒤 `workflow_dispatch` 로 **온전히 회복** (SPEC 8·9절) |
...
| `NOTION_TOKEN` | Internal Integration | 무기한 | Notion 통합 페이지. 루트 페이지에 통합 연결 필요. **권한: 콘텐츠 읽기·삽입 + 「이메일 주소를 제외한 사용자 정보 읽기」** | Notion 단계 예외, 멘션 시 `400` |
| `NOTION_ROOT_PAGE_ID` | 루트 페이지 | — | — | 월 페이지 생성 실패 |
| `NOTION_USER_ID` | 알림 멘션 대상 사용자 UUID | 무기한 (상수) | `GET /v1/users` 의 `type: person` (SPEC 12절) | `400 Could not find user`, 또는 알림이 사람 대신 봇에게 |

**모든 자격 증명이 무기한이다.** v1.6에서 카카오를 걷어내며 60일 만료 토큰과 그 회전 배관이 사라졌다 —
정기적으로 갱신해야 하는 값이 더는 없다. 남은 실패 지점은 만료가 아니라 **권한 설정**이다 (위 `NOTION_TOKEN` 행).
...
정기 점검 시점: 마지막 확인일(SPEC 7절 "2026-09-08", 8·11·12절 "2026-09-09")에서 한 달 이상 지났을 때,
또는 `summary: gemini 0` 이나 알림 미도착이 원인 불명일 때. **알림 항목(#6)의 근거는 공식 문서가 아니라 실측**이므로
Notion 이 동작을 바꾸면 조용히 깨진다 — 점검 주기를 지키는 이유다.
