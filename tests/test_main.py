...
def services(tmp_path):
    ...
    s = Services(
        ...
        kakao_refresh=lambda: Tokens("acc", None),
        kakao_send=lambda tok, text, link: rec.sent.append((tok, text, link)),
        persist_refresh_token=rec.persisted.append,
        log_file=tmp_path / "logs" / "last_run.txt",
    )
    s.rec, s.fake_notion = rec, notion   # 테스트 편의
    return s
...
def test_full_run_reproduces_fixture_week(services):
    ...
    # 카톡: 본문 + 토글 앵커 링크
    assert services.rec.sent == [("acc", "📡 이번 주 테크 브리핑 · 국내 3 / 해외 5\n8월 2주 (08/10~08/16)", result.notion_link)]
...
def test_second_run_is_idempotent(services):
    ...
    assert len(services.fake_notion.toggles["page-2026-08"]) == 1
    assert len(services.rec.sent) == 1


def test_dry_run_writes_nothing(services, capsys):
    result = run(services, now=NOW, dry_run=True)
    assert result.status == "dry_run" and services.fake_notion.toggles == {} and services.rec.sent == []
...
def test_kakao_failure_keeps_notion_and_reports(services):
    services.kakao_refresh = lambda: (_ for _ in ()).throw(RuntimeError("KOE319"))
    result = run(services, now=NOW)
    assert result.status == "published" and result.failures == ("kakao",) and result.exit_code == 1
    assert len(services.fake_notion.toggles["page-2026-08"]) == 1


def test_rotated_refresh_token_is_persisted(services):
    services.kakao_refresh = lambda: Tokens("acc", "new-refresh")
    run(services, now=NOW)
    assert services.rec.persisted == ["new-refresh"]
