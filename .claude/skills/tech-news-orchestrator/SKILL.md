...
| 필요 기능 | 사용 도구 |
|----------|----------|
| 팀원 생성 | `Agent(subagent_type: "{에이전트 정의 이름}", name: "{같은 이름}", model: "opus", prompt: ...)` — 백그라운드로 실행되며 완료 시 리더에게 알림이 온다. **주의(2026-09-09 프로브):** 커스텀 타입 스폰은 거부되지 않지만, 정의 파일이 세션 시작 이후에 만들어졌으면 시스템 프롬프트로 **로드되지 않는다**. 그래서 팀원 프롬프트 첫 줄에 **항상** `.claude/agents/{이름}.md` 를 Read 하고 그 역할로 행동하라고 지시한다 — 정의가 로드됐든 아니든 결과가 같아진다 |
...
- 폴백: 커스텀 `subagent_type` 이 거부되는 환경이면 `general-purpose` 로 스폰한다. `name` 은 그대로 정의 이름을 쓴다. 정의 파일 Read 지시는 폴백일 때만이 아니라 **항상** 프롬프트 첫 줄에 넣는다 (위 "주의")
...
(1) **첫 줄:** "먼저 `/home/robot/tech_news/.claude/agents/{이름}.md` 를 Read 하고 그 정의대로 행동하라. 거기 적힌 스킬 파일도 읽어라" — 정의가 시스템 프롬프트로 로드됐는지 팀원은 알 수 없으므로 무조건 넣는다, (2) 입력 파일 절대 경로, (3) 출력 파일 절대 경로, (4) 통신 상대 팀원 이름과 "첫 `SendMessage` 전 `ToolSearch("select:SendMessage")`", (5) 사용자 요청 원문.
